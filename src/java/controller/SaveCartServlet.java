        ...
        try (PrintWriter out = response.getWriter()) {
          ...
          HashMap<FlowerDTO, Integer> cart = (HashMap<FlowerDTO, Integer>) session.getAttribute("cart");
           ...
           if(cart!=null && !cart.isEmpty()){
               int rs=OrderDAO.inserOrder(1, userID, Double.parseDouble(total), cart);
               session.removeAttribute("cart");
               request.setAttribute("message", "Đơn hàng đang được xử lý");
               OrderDTO ord = OrderDAO.lastOrd(userID);
               request.setAttribute("ord", ord);
                request.getRequestDispatcher("MainServlet?action=OrderInfor").forward(request, response);
           }
        }
    }
